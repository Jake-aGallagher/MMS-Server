...
export async function getFile(req: Request, res: Response) {
    ...
    downloadFile(clientId, res, req.params.fileid, 'attachment');
}

export async function getImage(req: Request, res: Response) {
    const clientId = req.params.clientid
    downloadFile(clientId, res, req.params.imageid, 'inline');
}

async function downloadFile(clientId: string, res: Response, id: string, type: 'attachment' | 'inline') {
    try {
        const hashIds = new Hashids('file', 8);
        const imageId = hashIds.decode(id)[0];
        const fileDetails = await Files.getFileDetails(clientId, imageId);
...
export async function getFilesForModel(req: Request, res: Response) {
    try {
        ...
        const files = await getFileIds(req.clientId, model, id);
...
export async function postFile(req: Request, res: Response) {
    try {
        ...
        const returnData = await uploadFile(req.clientId, req.file);
        await postFileMappings(req.clientId, 'file', [returnData.insertId], req.body.model, req.body.id);
...
async function uploadFile(clientId: string, file: Express.Multer.File) {
    const blobName: string = clientId.toUpperCase() + '/' + uuidv4() + '___' + file.originalname.toLowerCase().split(' ').join('-');
    const { blockBlobClient, fileSize } = await getBlockBlobClient(blobName);
    const stream = file.buffer;
    await blockBlobClient.upload(stream, stream.length);
    return await Files.postFile(clientId, { originalname: file.originalname, mimetype: file.mimetype, blobName, blobSize: fileSize.toString() });
}
...
export async function postSignature(req: Request, res: Response) {
    try {
        ...
        const response = await Files.postFile(req.clientId, { originalname: 'signature.jpg', mimetype: 'image/jpeg', blobName: fileName, blobSize: decodedData.length.toString() });
        const hashIds = new Hashids('file', 8);
...
export async function deleteFile(req: Request, res: Response) {
    try {
        ...
        const fileDetails = await Files.getFileDetails(req.clientId, fileId);
        const { blockBlobClient } = await getBlockBlobClient(fileDetails.location_name);
        await blockBlobClient.delete();
        await Files.deleteFile(req.clientId, fileId);
        res.status(200).send();
    } catch (err) {
        console.log(err);
        res.status(500).send();
